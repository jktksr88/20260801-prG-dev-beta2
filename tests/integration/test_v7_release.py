...
def test_build_endpoint_and_versioned_browser_assets(client):
    ...
    assert build.json()["build"] == "7.0.0"
    index = client.get("/")
    assert index.status_code == 200
    assert "app.v7.js" in index.text
    assert "styles.v7.css" in index.text
    assert index.headers["x-groe-build"] == "7.0.0"
...
def test_static_ui_contains_all_client_feedback_components():
    js = Path("backend/app/static/assets/app.v7.js").read_text()
    css = Path("backend/app/static/assets/styles.v7.css").read_text()
