...
GROE deploys as one Dockerized Render web service and one Render PostgreSQL database. The frontend is compiled during the Docker build and copied into `backend/app/static`; FastAPI serves both the SPA and `/api/v1` from one origin.

## Runtime layers

1. **React presentation layer** — guided planner, custom plot editor, comparison, SVG result map, saved gardens, sharing and diary.
...
FastAPI mounts `/assets` when a compiled Vite build exists and returns `index.html` for non-API paths. This supports public share URLs such as `/shared/{share_slug}` without a separate static-site service.
