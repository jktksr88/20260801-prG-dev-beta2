(() => {
      ...
      content = `
        ...
        <div class="dimension-card">
          <div class="dimension-inputs">
            ...
            <label class="field"><span>${esc(lt("Strongest sunlight direction", "Arah cahaya terkuat"))}</span><select data-field="sun-direction"><option value="north" ${input.plot.sun_direction === "north" ? "selected" : ""}>${esc(lt("North", "Utara"))}</option><option value="east" ${input.plot.sun_direction === "east" ? "selected" : ""}>${esc(lt("East", "Timur"))}</option><option value="south" ${input.plot.sun_direction === "south" ? "selected" : ""}>${esc(lt("South", "Selatan"))}</option><option value="west" ${input.plot.sun_direction === "west" ? "selected" : ""}>${esc(lt("West", "Barat"))}</option></select><small>${esc(lt("Choose where the strongest direct light enters the plot.", "Pilih dari sisi mana cahaya langsung terkuat masuk ke area."))}</small></label>
  ...
  function plotMap(plan) {
    ...
    const offset = 0.68;
    const outer = 0.68;
    const viewW = maxX + offset + outer;
    const viewH = maxY + offset + outer;
    const points = polygonPoints(boundary, offset);
    ...
    const access = layout.access_zone ? `<polygon points="${polygonPoints(layout.access_zone, offset)}" fill="#d8cbb8" opacity=".94" stroke="#ae9d85" stroke-width=".018"></polygon>` : "";
    const visiblePlacements = (layout.placements || []).filter(p => p.structure_type !== "rack_pot");
    const placements = visiblePlacements.map(p => {
      const active = state.selectedCrop === p.slug;
      const x = Number(p.x_m || 0) + offset;
      const y = Number(p.y_m || 0) + offset;
    ...
    const modules = (layout.vertical_modules || []).map(module => {
      if (module.type !== "tiered_rack") return "";
      const x = Number(module.x_m ?? 0) + offset;
      const y = Number(module.y_m ?? 0) + offset;
      const w = Number(module.module_width_m || .65);
      const d = Number(module.module_depth_m || .4);
      const rows = [3,2,1].map((tier, rowIndex) => {
        const assignment = (module.assignments || []).find(item => Number(item.tier) === tier);
        const rowY = y + rowIndex * (d / 3);
        if (!assignment) return `<rect x="${x+.025}" y="${rowY+.012}" width="${w-.05}" height="${d/3-.024}" rx=".018" fill="#9a8877" opacity=".42"></rect><text x="${x+w-.055}" y="${rowY+d/6+.018}" text-anchor="end" font-size=".045" fill="#f7f1e8">T${tier}</text>`;
        const crop = (plan.crops || []).find(item => item.slug === assignment.crop_slug) || {};
        const visual = cropVisual(assignment.crop_slug, crop.category);
        return `<rect x="${x+.025}" y="${rowY+.012}" width="${w-.05}" height="${d/3-.024}" rx=".018" fill="#7f6c5c" stroke="#f7f1e8" stroke-width=".009"></rect><circle cx="${x+.095}" cy="${rowY+d/6}" r=".046" fill="${visual.color}"></circle><text x="${x+.095}" y="${rowY+d/6+.018}" text-anchor="middle" font-size=".038" font-weight="900" fill="#fff">${esc(visual.code)}</text><text x="${x+w-.055}" y="${rowY+d/6+.018}" text-anchor="end" font-size=".045" font-weight="800" fill="#fff">T${tier}</text>`;
      }).join("");
      return `<g class="map-module rack-module"><rect x="${x}" y="${y}" width="${w}" height="${d}" rx=".035" fill="#57483d" stroke="#fff" stroke-width=".018"></rect>${rows}</g>`;
    }).join("");

    const compost = layout.compost ? `<rect x="${Number(layout.compost.x_m)+offset}" y="${Number(layout.compost.y_m)+offset}" width="${layout.compost.width_m}" height="${layout.compost.height_m}" rx=".04" fill="#5d4936" stroke="#cdb28f" stroke-width=".025"></rect><text x="${Number(layout.compost.x_m)+offset+layout.compost.width_m/2}" y="${Number(layout.compost.y_m)+offset+layout.compost.height_m/2+.02}" text-anchor="middle" font-size=".08" fill="#fff">C</text>` : "";

    const direction = layout.sun_direction || state.input?.plot?.sun_direction || "north";
    const labels = {
      north: { short: "N", en: "North", id: "Utara" },
      east: { short: "E", en: "East", id: "Timur" },
      south: { short: "S", en: "South", id: "Selatan" },
      west: { short: "W", en: "West", id: "Barat" }
    };
    const dir = labels[direction] || labels.north;
    const plotLeft = offset, plotTop = offset, plotRight = offset + maxX, plotBottom = offset + maxY;
    let sx, sy, tx, ty, labelX, labelY, anchor = "middle";
    if (direction === "east") { sx = plotRight + .38; sy = plotTop + maxY/2; tx = plotRight + .04; ty = sy; labelX = sx; labelY = sy + .21; }
    else if (direction === "south") { sx = plotLeft + maxX/2; sy = plotBottom + .38; tx = sx; ty = plotBottom + .04; labelX = sx; labelY = sy + .21; }
    else if (direction === "west") { sx = plotLeft - .38; sy = plotTop + maxY/2; tx = plotLeft - .04; ty = sy; labelX = sx; labelY = sy + .21; }
    else { sx = plotLeft + maxX/2; sy = plotTop - .38; tx = sx; ty = plotTop - .04; labelX = sx; labelY = sy - .15; }

    const compassX = viewW - .30;
    const compassY = viewH - .30;
    return `
      <div class="map-wrap"><svg class="plot-svg" viewBox="0 0 ${viewW} ${viewH}" role="img" aria-label="Scaled garden layout with sunlight direction">
        <defs>
          ...
          <marker id="sun-arrow-head" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 Z" fill="#c99816"></path></marker>
        </defs>
        ...
        <g class="sun-source"><circle cx="${sx}" cy="${sy}" r=".10" fill="#f3c94f" stroke="#c99816" stroke-width=".014"></circle><text x="${sx}" y="${sy+.035}" text-anchor="middle" font-size=".12">☀</text><line x1="${sx}" y1="${sy}" x2="${tx}" y2="${ty}" stroke="#c99816" stroke-width=".025" marker-end="url(#sun-arrow-head)"></line><text x="${labelX}" y="${labelY}" text-anchor="${anchor}" font-size=".065" font-weight="800" fill="#4b392b">${esc(lt(`Strongest light: ${dir.en}`, `Cahaya terkuat: ${dir.id}`))}</text></g>
        <g class="compass" transform="translate(${compassX} ${compassY})"><circle r=".17" fill="#fffaf0" stroke="#6d5a49" stroke-width=".012"></circle><line x1="0" y1="-.11" x2="0" y2=".11" stroke="#6d5a49" stroke-width=".012"></line><line x1="-.11" y1="0" x2=".11" y2="0" stroke="#6d5a49" stroke-width=".012"></line><text x="0" y="-.19" text-anchor="middle" font-size=".055" font-weight="900">N</text><text x=".19" y=".02" text-anchor="middle" font-size=".055" font-weight="900">E</text><text x="0" y=".23" text-anchor="middle" font-size=".055" font-weight="900">S</text><text x="-.19" y=".02" text-anchor="middle" font-size=".055" font-weight="900">W</text></g>
        <text x="${offset}" y="${viewH-.08}" font-size=".072" fill="#4b392b">1 unit = 1 metre</text>
      </svg></div>`;
  }
  ...
  function verticalAssignmentSummary(plan) {
    const modules = plan.layout?.vertical_modules || [];
    const lines = [];
    const hanging = modules.find(module => module.type === "hanging_pot");
    if (hanging) {
      const label = state.lang === "id" ? hanging.name_id : hanging.name_en;
      lines.push(`<span><b>${esc(lt("Hanging pot", "Pot gantung"))}</b>${esc(label || hanging.crop_slug)}</span>`);
    }
    const rack = modules.find(module => module.type === "tiered_rack" && (module.assignments || []).length);
    if (rack) {
      const assignments = [...rack.assignments].sort((a,b) => Number(a.tier)-Number(b.tier));
      for (const assignment of assignments) {
        const label = state.lang === "id" ? assignment.name_id : assignment.name_en;
        lines.push(`<span><b>${esc(lt(`Rack Tier ${assignment.tier}`, `Rak Tingkat ${assignment.tier}`))}</b>${esc(label || assignment.crop_slug)}</span>`);
      }
    }
    if (!lines.length) return "";
    return `<div class="vertical-assignment-summary"><div><b>${esc(lt("Vertical placement assignments", "Penempatan vertikal"))}</b><small>${esc(lt("Every item below is one real plant from the calculated plan.", "Setiap item di bawah adalah satu tanaman nyata dari perhitungan rencana."))}</small></div><div>${lines.join("")}</div></div>`;
  }
    ...
    return `
      <main class="page detail-page">
        ...
        <section class="layout-panel map-first">
          ...
          ${cropLegend(plan)}
          ${mapLegend(plan)}
          ${verticalAssignmentSummary(plan)}
          ${potSummary(plan)}
  ...
  function diary() {
    ...
    const entries = state.diaryEntries.map(item => {
      const detected = Array.isArray(item.detected_crops) && item.detected_crops.length
        ? item.detected_crops
        : item.detected_crop_name
          ? [{ name: item.detected_crop_name }]
          : [];
      const cropStatus = detected.length
        ? `<div class="detected-crop-list"><span>${esc(lt(detected.length > 1 ? "Detected plants" : "Detected plant", detected.length > 1 ? "Tanaman terdeteksi" : "Tanaman terdeteksi"))}</span>${detected.map(crop => `<b>${esc(crop.name)}</b>`).join("")}</div>`
  ...
  function footer() {
    return `<footer><b>GROE</b><span>Grow Resources in Omni-sustainable Environment</span><small class="build-badge">Beta build ${BUILD} · multi-crop diary + vertical map release</small></footer>`;
  }
  ...
  async function submitDiary() {
    ...
    try {
      ...
      const entry = {
        ...
        detected_crops: advice.detected_crops || []
      };
  ...
  root.addEventListener("input", event => {
    ...
    else if (field === "sun-direction") state.input.plot.sun_direction = value;
    ...
    if (field === "plot-length" || field === "plot-width" || field === "container-depth" || field === "sun-direction") render();
  });
  ...
  root.addEventListener("change", event => {
    ...
    else if (field === "sun-direction") { state.input.plot.sun_direction = event.target.value; render(); }
  });
