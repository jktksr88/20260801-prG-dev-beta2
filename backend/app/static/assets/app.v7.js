(() => {
  ...
  function cropLegend(plan) {
    ...
    return `<div class="crop-map-key"><b>${esc(lt("Plant key", "Kunci tanaman"))}</b><div>${crops.map(crop => {
      ...
      const label = state.lang === "id" ? crop.name_id : crop.name_en;
      return `<span style="--crop:${attr(visual.color)}"><i>${esc(visual.emoji)}</i>${esc(label)}</span>`;
    }).join("")}</div></div>`;
  }
  ...
  function footer() {
    return `<footer><b>GROE</b><span>Grow Resources in Omni-sustainable Environment</span><small class="build-badge">Beta build ${BUILD}</small></footer>`;
  }
  ...
  async function searchLocations(query) {
    ...
    try {
      ...
      if (state.input.location.city.trim() === cleaned) {
        state.locationSuggestions = data.items || [];
        state.locationSearched = true;
      }
    } catch (_) {
      state.locationSuggestions = [];
      state.locationSearched = true;
      state.error = lt("Live location search is temporarily unavailable. GROE will still use its Indonesia fallback list where possible.", "Pencarian lokasi live sementara tidak tersedia. GROE tetap memakai daftar cadangan Indonesia bila tersedia.");
    } finally {
      state.locationLoading = false;
      render();
