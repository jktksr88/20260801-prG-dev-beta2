(() => {
    ...
    const placements = (layout.placements || []).map(p => {
      ...
      return `
        <g class="placement ${active ? "active" : ""}" data-action="open-crop-guide" data-slug="${attr(p.slug)}" tabindex="0" role="button">
          ${shape}
          <text x="${cx}" y="${cy + fontSize*.24}" text-anchor="middle" font-size="${Math.max(.075,fontSize*.66)}" font-family="system-ui" font-weight="900" fill="#fff">${esc(visual.code)}</text>
  ...
  function footer() {
    return `<footer><b>GROE</b><span>Grow Resources in Omni-sustainable Environment</span><small class="build-badge">Beta build ${BUILD} · client feedback release</small></footer>`;
  }
